...
// Generic admonition editor component - shows styled placeholder
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 text-center text-gray-500 dark:text-gray-400">
        <p className="text-sm">{config.label} content</p>
        <p className="text-xs mt-1 italic">Switch to &quot;Raw Markdown&quot; tab to edit</p>
      </div>
    </div>
  );
};
...
// Card directive editor - shows placeholder
...
// Tab-set directive editor - shows placeholder
const TabSetEditor: React.FC = () => (
  <div className="my-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
    ...
    <div className="p-4 text-center text-gray-500 dark:text-gray-400">
      <p className="text-sm">Tab set content</p>
      <p className="text-xs mt-1 italic">Switch to &quot;Raw Markdown&quot; tab to edit</p>
    </div>
  </div>
);

// Tab-item directive editor - shows placeholder
const TabItemEditor: React.FC = () => (
  <div className="my-2 rounded border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 overflow-hidden">
    ...
    <div className="p-3 text-center text-gray-500 dark:text-gray-400">
      <p className="text-xs italic">Edit in Raw Markdown</p>
    </div>
  </div>
);

// List-table directive editor - shows placeholder
const ListTableEditor: React.FC = () => (
  <div className="my-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
    ...
    <div className="p-4 text-center text-gray-500 dark:text-gray-400">
      <p className="text-sm">Table content</p>
      <p className="text-xs mt-1 italic">Switch to &quot;Raw Markdown&quot; tab to edit</p>
    </div>
  </div>
);

// Simple display component for code-based directives (mermaid, math, code)
// These show a placeholder in Rich Editor mode - edit in Raw Markdown for full control
interface CodeDirectiveDisplayProps {
  label: string;
  icon: React.ReactNode;
  bgColor: string;
  textColor: string;
...
const CodeDirectiveDisplay: React.FC<CodeDirectiveDisplayProps> = ({
  label,
  icon,
  bgColor,
  textColor,
  ...
  <div className="my-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
    <div className={`flex items-center gap-2 px-4 py-2 ${bgColor} border-b border-gray-200 dark:border-gray-600 ${textColor} font-semibold`}>
      {icon}
      <span>{label}</span>
    </div>
    <div className="p-4 bg-gray-50 dark:bg-gray-900 text-center text-gray-500 dark:text-gray-400">
      <p className="text-sm">{description}</p>
      <p className="text-xs mt-1 italic">Switch to &quot;Raw Markdown&quot; tab to edit code content</p>
    </div>
  </div>
);

// Mermaid diagram editor - shows placeholder (edit in raw mode)
const MermaidEditor: React.FC = () => (
  <CodeDirectiveDisplay
...
// Figure directive editor - shows placeholder
...
// Math directive editor - shows placeholder (edit in raw mode)
...
// Code directive editor - shows placeholder (edit in raw mode)
...
// Exercise directive editor - shows placeholder
const ExerciseEditor: React.FC = () => (
  <CodeDirectiveDisplay
    ...
    description="Exercise block"
  />
);

// Solution directive editor - shows placeholder
...
// Sidebar directive editor - shows placeholder
...
// Margin note directive editor - shows placeholder
const MarginEditor: React.FC = () => (
  <CodeDirectiveDisplay
    ...
    bgColor="bg-amber-50 dark:bg-amber-900/20"
    textColor="text-amber-700 dark:text-amber-300"
    description="Margin note"
  />
);

// Generic fallback directive editor - shows placeholder
...
/**
 * Add a default language to code blocks that don't have one.
 * MDXEditor fails to parse code blocks without a language specifier.
 * This handles various edge cases:
 * - ``` (empty)
 * - ```  (with trailing spaces)
 * - ``` followed by newline
 */
function addDefaultLanguageToCodeBlocks(content: string): string {
  if (!content) return content;

  let result = content;

  // Pattern 1: ``` at start of line with nothing after (or just whitespace)
  result = result.replace(/^```\s*$/gm, '```text');

  // Pattern 2: ``` followed immediately by newline (no language)
  // This catches cases where the AI puts the code on the next line immediately
  result = result.replace(/^```\n/gm, '```text\n');

  // Pattern 3: Handle code blocks that might have only whitespace as "language"
  // e.g., "```   \ncode" should become "```text\ncode"
  result = result.replace(/^```[ \t]+\n/gm, '```text\n');
